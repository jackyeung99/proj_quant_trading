...
from qbt.core.types import ModelInputs, ModelBundle
from qbt.storage.storage import Storage
from qbt.storage.paths import StoragePaths
from qbt.config.specs import StrategySpec, DatasetSpec
...
from qbt.utils.dates import _to_utc, _stamp_asof_utc
...
def _slice_model_inputs(inputs: ModelInputs, idx: pd.Index) -> ModelInputs:
    ...
    return ModelInputs(
        ...
        asset_features={name: panel.loc[idx] for name, panel in inputs.asset_features.items()},
...
def enforce_bundle_schema(bundle: ModelBundle, inputs: ModelInputs) -> ModelInputs:
    ...
    required_asset_features = list(bundle.asset_feature_cols or [])
    required_global_features = list(bundle.global_feature_cols or [])
    ...
    if inputs.global_features is None:
        if required_global_features:
            raise ValueError(f"Missing global features at inference: {required_global_features}")
        global_features = pd.DataFrame(index=ret.index)
    else:
        missing_global = [
    ...
    idx = ret.index
    for _, panel in asset_features.items():
...
def signal(
    ...
    *,
    ...
    universe = strategy.universe or "default"

    training_cfg = strategy.training or {}
    execution_cfg = strategy.execution or {}
    params_cfg = strategy.params or {}

    market_tz = execution_cfg.get("market_tz", "America/New_York")
    cutoff_hour = float(execution_cfg.get("cutoff_hour", 16.0))

    retrain_freq = training_cfg.get("retrain_freq", "1D")
    lookback = int(training_cfg.get("train_lookback_bars", 252))
    min_train = int(training_cfg.get("min_train_bars", 200))
    ...
    data_adapter = data_adapter or WidePrefixDataAdapter(
        storage=live_storage.storage,
        paths=paths
    )

    strat = create_strategy(strategy.strategy_class)
    req_asset = strat.required_asset_features(strategy)
    req_global = strat.required_global_features(strategy)

    bundle = live_storage.read_model(strategy.strategy_name, universe)
    meta = live_storage.read_model_meta(strategy.strategy_name, universe) or None
    ...
    if do_train:
        logger.info(
            "Retrain triggered | strategy=%s universe=%s lookback=%s min_train=%s",
        ...
        strat.fit(train_inputs, strategy)
        ...
        train_start_asof_utc = _stamp_asof_utc(
            train_start_session,
            market_tz=market_tz,
            hour=cutoff_hour,
        )
        train_end_asof_utc = _stamp_asof_utc(
            train_end_session,
            market_tz=market_tz,
            hour=cutoff_hour,
        )
        ...
        live_storage.write_model(
            ...
            bundle=bundle,
            meta=meta_out,
            snapshot=bool(training_cfg.get("snapshot_models", True)),
        )

        meta = meta_out
        logger.info("Model write complete | snapshot_id=%s", snapshot_id)

    else:
        logger.info(
            "Retrain skipped | strategy=%s universe=%s",
            strategy.strategy_name,
            universe,
        )
        if meta:
    ...
    w_ts = bundle.model.predict(inputs_now, strategy)
    ...
    live_storage.write_weights(
        ...
        universe=universe,
        latest_w=latest_w,
        snapshot=bool(training_cfg.get("snapshot_weights", True)),
    )
